...
public class MathExercises {
    ...
    * Output: index1=1, index2=2
     */
    public static void sumOfTwoNumbers(int[] numbers, int target){
        int howManyNumbers = numbers.length;
        int idxI = 0, idxJ;
        boolean found = false;
        while(!found && idxI < howManyNumbers){
            idxJ = idxI + 1;
            while(!found && idxJ < howManyNumbers) {
                if(numbers[idxI]+numbers[idxJ]==target){
                    idxI++; //+1 to avoid starting from 0 (per requirement above)
                    idxJ++; //+1 to avoid starting from 0 (per requirement above)
                    System.out.print("The index of the two numbers that sums "+target+" in "+ Arrays.toString(numbers) +" are: ");
                    System.out.println("index1="+idxI+", index2="+idxJ);
                    found = true;
                }
                idxJ++;
            }
